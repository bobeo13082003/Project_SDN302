...
.filter((q, qIndex) => qIndex !== questionIndex)
  ...
  const onAnswerClickMulti = (
    questionIndex: number,
    option: string
  ) => {
    if (!isSubmitted) {
      // Maintain a set of selected answers per question
      const selectedAnswersForQuestion = answers[questionIndex]
        ? answers[questionIndex].split(`/"'\\`).filter(Boolean)
        : [];
      const newSelectedAnswers = selectedAnswersForQuestion.includes(option)
        ? selectedAnswersForQuestion.filter((ans) => ans !== option) // Deselect
        : [...selectedAnswersForQuestion, option]; // Add selection
      setAnswers({
        ...answers,
        [questionIndex]: newSelectedAnswers.join(`/"'\\`),
      });
    }
  };
  ...
  const handleMultiAnswerSubmit = () => {
    ...
    filteredQuestions.forEach((question, index) => {
      const correctAnswers = question.correctAnswer.split(`/"'\\`).filter(Boolean); // Split into array
      const selectedAnswers = (answers[index] || "").split(`/"'\\`).filter(Boolean);
  
      if (answerMode === "text") {
        if (
          selectedAnswers.length === correctAnswers.length &&
         question.correctAnswer.split(`/"'\\`).join(",") == answers[index]
        ) {
          newScore += 1;
        }
      } else {
        // Existing logic for multi-choice
        if (
          selectedAnswers.length === correctAnswers.length &&
        ...
        renderItem={(question, Questionindex) => {
          ...
          return (
            <List.Item style={{ display: "flex", justifyContent: "center" }}>
              ...
              >
                <Title level={5} style={{ marginBottom: 100 }}>
                ...
                {(answerMode === "select" ||
                  answerMode === null ||
                  (answerMode === "random" &&
                    (randomValue * correctAnswers.length) % 2 === 1)) && (
                  <div className="options-container">
                    {[
                      ...
                      .map((option, idx) => {
                        ...
                        const isFulllySelected = isSelected == isCorrect;
                        return (
                          <div
                ...
                {(answerMode === "text" ||
                  (answerMode === "random" &&
                    (randomValue * correctAnswers.length) % 2 === 0)) && (
                  <>
                    <Input
                      ...
                      style={{
                        ...
                        borderColor:
                          showResults &&
                          answers[Questionindex] === correctAnswers.join(",")
                            ? "green"
                            : showResults
                            ? "red"
                            : "initial",
                    ...
                    {showResults && (
                      <div style={{ marginTop: 8 }}>
                        {isCorrectText ? (
                          ...
                          <Alert
                            message={`Incorrect! Correct answers: ${correctAnswers.join(
                              ","
                            )}`}
                            type="error"
                            showIcon
